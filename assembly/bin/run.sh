...
MAIN_CLASS=me.asu.test.AutoTestMain
...
APP_ARGS=/opt/account/unify-account-auto-test/service/conf/application.properties


# Add default JVM options here. You can also use JAVA_OPTS to pass JVM options to this script.
#-Xms128M -Xmx512M -Xss256K -XX:PermSize=96m -XX:MaxPermSize=128m
...
CLASSPATH=${APP_HOME}/conf:${APP_HOME}/lib/*:${APP_HOME}/*
...
start()
{
    ...
    nohup "$JAVACMD" $JAVA_OPTS  -cp $CLASSPATH $MAIN_CLASS  $APP_ARGS $@> ${OUT_FILE} 2>&1 &
    echo $! > ${RUNNING_PID}
    #popd > /dev/null
...
stop()
{
	...
	pid=`ps -ef|grep unify-account-auto-test|grep -v 'grep'|awk '{print $2}'`
